...
function [spec, Smin, Smax, T, F, s_init_abs] = myspectrogram_test(s, fs, T, w, nfft, Slim, alpha, cmap, cbar, type)
...
%    S(S<eps) = eps;                     % prevent zero spectral magnitude values 
    ...
    axis('xy');
    axis([0 N/fs  0 fs/2]);
   xlabel('time (s)', 'FontSize', 16, 'FontWeight', 'n');
   ylabel('frequency (Hz)', 'FontSize', 16, 'FontWeight', 'n');
